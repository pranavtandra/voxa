...
export default function Home() {
  ...
  const [addingWord,setAddingWord]=useState(false); const [newWord,setNewWord]=useState(""); const [newEmoji,setNewEmoji]=useState("✨");
  ...
  return <main className={`app ${animations?"":"no-motion"}`}>
    ...
    {page==="Communicate"&&<div className="communicate">
      ...
      <section className="board">
        ...
        {addingWord&&<form className="inline-maker" onSubmit={addWord}><div><h3>Add to {categories.find(c=>c[0]===category)?.[1]}</h3><button type="button" aria-label="Close" onClick={()=>setAddingWord(false)}>×</button></div><label>Word or phrase<input autoFocus value={newWord} onChange={e=>setNewWord(e.target.value)} placeholder="Type your own"/></label><fieldset className="emoji-picker"><legend>Choose an emoji <span aria-live="polite">Selected: {newEmoji}</span></legend><div>{emojiChoices.map(emoji=><button key={emoji} type="button" className={newEmoji===emoji?"selected":""} aria-label={`Choose ${emoji}`} aria-pressed={newEmoji===emoji} onClick={()=>setNewEmoji(emoji)}>{emoji}</button>)}</div></fieldset><button className="primary">Add word</button></form>}
