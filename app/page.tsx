...
export default function Home() {
  ...
  return <main className={`app ${animations?"":"no-motion"}`}>
    <header className="topbar">
      ...
      <div className="top-actions"><span className="private"><i/> Secure sync</span><button className={`account-button ${page==="Profile"?"active":""}`} onClick={()=>setPage("Profile")} aria-label={`Open ${accountName}'s profile`}><span className="account-avatar" aria-hidden>{accountName.slice(0,1).toUpperCase()}</span><span className="account-name">{accountName}</span><span aria-hidden>⌄</span></button><button className="logout" onClick={()=>void supabase.auth.signOut()}>Log out</button></div>
    </header>
